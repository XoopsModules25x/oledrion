<div class="oledrion-block">
		...
		<tr>
		  ...
		  <tr>
		    <td class="page-curl_01">
				...
				<div class="oledrion_productprice"><br /><{if $product.product_stock > 0 }><{$smarty.const._OLEDRION_PRICE}> <a href="<{$smarty.const.OLEDRION_URL}>caddy.php?op=addproduct&product_id=<{$product.product_id}>" title="<{$smarty.const._OLEDRION_ADD_TO_CART}>"><{if $product.product_discount_price_ttc != ''}><s><{$product.product_price_ttc}></s> <{$product.product_discount_price_ttc}><{else}><{$product.product_price_ttc}><{/if}> <img src="<{$smarty.const.OLEDRION_IMAGES_URL}>cartadd.png" alt="<{$smarty.const._OLEDRION_ADD_TO_CART}>" border="0" /></a><{else}><{$block.nostock_msg}><{/if}></div>
			</td>
		  </tr>
		</table>
			</td>
		...
		<{if $block.showMore }>
		<br />
		<div align='center'><a href="<{$smarty.const.OLEDRION_URL}>recommended.php" title="<{$smarty.const._OLEDRION_SEEMORE_RECOMM}>"><{$smarty.const._OLEDRION_SEEMORE_RECOMM}></a></div>
		<{/if}>
</div>