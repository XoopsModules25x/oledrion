        ...
        <table width='100%' cellspacing='1' cellpadding='3' border='0' class='outer'>
            <tr class="<{cycle values='even,odd'}>">
                <td><select name='searchField' id='searchField'
                            class="selectLists"><{html_options selected=$searchFieldSelected options=$searchField}></select>
                </td>
                <td><select name='searchCriteria' id='searchCriteria'
            ...
            <tr class="<{cycle values='even,odd'}>">
                <td><{$smarty.const._AM_OLEDRION_DISCOUNT_VENDOR}><br><select name='searchVendor' id='searchVendor'
                                                                              class="selectLists"><{html_options selected=$searchVendorSelected options=$searchVendor}></select>
                </td>
        ...
        <table width='100%' cellspacing='1' cellpadding='3' border='0' class='outer'>
            ...
            <{foreach item=product from=$products}>
                <tr class="<{cycle values='even,odd'}>">
                    ...
                    <td class='left'><a href="<{$product.product_url_rewrited}>"
                                        target="_blank"><{$product.product_title}></a></td>
                    <td class='center'>
                        <{if $mutipleSelect}>
                            <span style="cursor: pointer; text-decoration: underline;"
                                  onClick="addValuesToCaller(<{$product.product_id}>, '<{$product.product_title_javascript}>')"><{$smarty.const._AM_OLEDRION_ADD}></span>
                        <{else}>
                            <span style="cursor: pointer; text-decoration: underline;"
                                  onClick="replaceValueFromCaller(<{$product.product_id}>, '<{$product.product_href_title|escape:"quotes"}>')"><{$smarty.const._AM_OLEDRION_REPLACE}></span>
                        <{/if}>
                    </td>
                </tr>
            <{/foreach}>
