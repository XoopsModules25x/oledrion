   ...
   <div class="pad2">
	...
	<{if $op == 'default'}>
		...
		<{foreach item=list from=$lists}>
			<tr class="<{cycle values='even,odd'}>">
				<td class='left'><a href="<{$list.list_url_rewrited}>" target="_blank" title="<{$list.list_href_title}>"><{$list.list_title}></a></td>
				<td class='center'><{$list.list_type_description}></td>
				<td class='center'><{$list.list_formated_date}></td>
				<td class='center'><a href="<{$baseurl}>?op=edit&list_id=<{$list.list_id}>"><img src="<{xoModuleIcons16 edit.png}>" alt="<{$smarty.const._EDIT}>" title="<{$smarty.const._EDIT}>" /></a>  <a href="<{$baseurl}>?op=delete&list_id=<{$list.list_id}>"><img src="<{xoModuleIcons16 delete.png}>" alt="<{$smarty.const._DELETE}>" title="<{$smarty.const._DELETE}>" /></a></td>
			</tr>
		<{/foreach}>
		<tr class="<{cycle values="even,odd"}>">
			<td colspan="4" class="center">
				<div class="frmAddList">
					<form method="post" action="<{$baseurl}>" name="frmAddList" id="frmAddList"><input type="hidden" name="op" id="op" value="addList" /><input type="submit" name="btngo" id="btngo" value="<{$smarty.const._OLEDRION_CREATE_NEW_LIST}>" /></form>
			   </div>
			</td>
		</tr>
		</table>
	...
	<{elseif $op == 'addProduct'}>
		...
		<{else}>
			...
			<{if $message != ''}><div class="oledrion_message"><{$message}></div><{/if}>
