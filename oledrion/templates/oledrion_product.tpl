<div class="oledrion">
	...
	<tr>
	    ...
	    <td class="oledrion_productdescription">
	  		...
	  		<tr>
				<td class="top width45">
					...
					<div class="oledrion_rating">
						<div class="floatleft"><{$smarty.const._OLEDRION_RATINGC}> <{$product.product_rating_formated}> (<{$product.product_votes_count}>)</div>
						<div class="vote">
							<input value="<{$product.product_rating_formated}>" step="1" id="backing<{$product.product_id}>" type="range">
							<div class="floatleft rateit" data-url="<{$smarty.const.OLEDRION_URL}>ajax.php?op=rate" data-product="<{$product.product_id}>" data-rateit-backingfld="#backing<{$product.product_id}>" data-rateit-resetable="false" data-rateit-ispreset="true" data-rateit-min="0" data-rateit-max="10"></div>
						</div>
                  <{if $userCanRate}>
						<script type="text/javascript">
					...
					<div class="oledrion_productthumb-big">
							<img src="<{$product.product_image_full_url}>" alt="<{$product.product_href_title}>" />
					</div>
					<{/if}>
				</td>
				<td class="top width45">
					...
					<{if $product_joined_manufacturers != ''}>
						<div class="oledrion_productauthor_view-product">
							<img src="<{$smarty.const.OLEDRION_IMAGES_URL}>author.png" alt="<{$product_joined_manufacturers}>" /><span class="oledrion_productdescription-contentTitles"><{$smarty.const._OLEDRION_BY}></span> <{$product_joined_manufacturers}>
						</div>
					<{/if}>
					<!-- Price box -->
           ...
           <{if $product.product_discount_price_ttc != ''}>
							<s><{$product.product_price_ttc_long}> </s> <{$product.product_discount_price_ttc}>
							<{else}>
							<{$product.product_price_ttc_long}>
							<{/if}>
							</span>
							<a href="<{$smarty.const.OLEDRION_URL}>caddy.php?op=addproduct&product_id=<{$product.product_id}>" title="<{$smarty.const._OLEDRION_ADD_TO_CART}>"><img src="<{$smarty.const.OLEDRION_IMAGES_URL}>cartadd.png" alt="" /></a>
							<{if $product.product_ecotaxe != ''}>
							<div class="oledrion_view-product_price_ecotaxe"><span class="oledrion_productdescription-contentTitles"><{$smarty.const._OLEDRION_ECOTAXE}></span> : <{$product.product_ecotaxe_formated}></div>
							<{/if}>
							<{else}>
								...
								<form method="post" name="frmCart" id="frmCart" action="<{$smarty.const.OLEDRION_URL}>caddy.php?op=addproduct&product_id=<{$product.product_id}>">
									<table class="tablefix">
										<tr>
											...
											<td>
												<span class="oledrion_productdescription-contentTitles"><{$smarty.const._OLEDRION_PRICE}></span> : <{if $product.product_discount_price_ttc != ''}>
												<s><{$product.product_price_ttc}> </s>  <b><{$product.product_discount_price_ttc}></b>
												<{else}>
												<{$product.product_price_ttc_long}>
												<{/if}>
												...
												<div class="error" style="display: none;">
													<br /><img src="<{$smarty.const.OLEDRION_IMAGES_URL}>warning.png" width="24" height="24" style="float:left; margin: -5px 10px 0 0; " /><span></span>.<br clear="all" />
												</div>
											</td>
										</tr>
									</table>
					...
					<{if $product.product_attachment != ''}>
						<a href="<{$smarty.const.OLEDRION_ATTACHED_FILES_URL}>/<{$product.product_attachment}>" target="_blank" ><img src="<{$smarty.const.OLEDRION_IMAGES_URL}>attach.gif" alt="<{$smarty.const._OLEDRION_ATTACHED_FILE}>" width="9"  height="15" /> <{$smarty.const._OLEDRION_ATTACHED_FILE}></a>
					<{/if}>
					<{if $currentUserId > 0}>
				...
				<td colspan='2'>
						...
						<table width="100%" cellspacing="0" class="tablefix">
		  				...
		  				<tr>
		    				...
		    				<td class="oledrion_catdescription">
			    				<{if $product.product_summary != ''}><div class="oledrion_productssummary_view-product"><h3><{$smarty.const._OLEDRION_SUMMARY}></h3><{$product.product_summary}></div><{/if}>
			    				<{if $product.product_description!= ''}><div class="oledrion_description_view-product"><h3><{$smarty.const._OLEDRION_DESCRIPTION}></h3><{$product.product_description}></div><{/if}>
		    				</td>
					...
					<{if $product.product_width != '' || $product.product_weight != 0 || $product.product_url != '' || $product.product_url2 != '' || $product.product_url3 != ''}>
		        		<div class="oledrion_otherinf">
							...
							<{if $product.product_url2 != ''}><div><span class="oledrion_productdescription-contentTitles"><{$smarty.const._OLEDRION_SITEURL}></span>: <a href="<{$product.product_url2}>" target="_blank"><{$smarty.const._OLEDRION_URL}></a></div><{/if}>
							<{if $product.product_url3 != ''}><div><span class="oledrion_productdescription-contentTitles"><{$smarty.const._OLEDRION_SITEURL}></span>: <a href="<{$product.product_url3}>" target="_blank"><{$smarty.const._OLEDRION_URL}></a></div><{/if}>
						</div>
					<{/if}>
	...
	<{if count($product_related_products) > 0}>
		<div id="oledrion_related">
			<h2><img src="<{$smarty.const.OLEDRION_IMAGES_URL}>icon-product-person.png" alt="<{$smarty.const._OLEDRION_CART}>" /><{$smarty.const._OLEDRION_RELATED_PRODUCTS}></h2>
			<table class='center oledrion_categorylist'>
			<tr>
				<{foreach item=oneitem from=$product_related_products}>
					<td class="center top">
			    		<{include file="db:oledrion_product_box.html" product=$oneitem}>
		    		</td>
		   			<{if $oneitem.count is div by $columnsCount}>
		    			</tr><tr>
		    		<{/if}>
				<{/foreach}>
			</tr>
	...
	<div id="oledrion_otherproducts">
		<{if $previous_product_id != 0 || $next_product_id != 0}>
			<h2><img src="<{$smarty.const.OLEDRION_IMAGES_URL}>icon-product-person.png" alt="<{$smarty.const._OLEDRION_OTHER_PRODUCTS}>" /><{$smarty.const._OLEDRION_OTHER_PRODUCTS}></h2>
		<{/if}>
		<{if $previous_product_id != 0}>
			<br /><a href="<{$previous_product_url_rewrited}>" title="<{$previous_product_href_title}>"><img src="<{$smarty.const.OLEDRION_IMAGES_URL}>go-previous.png" alt="<{$previous_product_title}>" /> <{$smarty.const._OLEDRION_PREVIOUS_PRODUCT}>: <{$previous_product_title}></a>
		<{/if}>
		<{if $next_product_id != 0}>
			<br /><a href="<{$next_product_url_rewrited}>" title="<{$next_product_href_title}>"><img src="<{$smarty.const.OLEDRION_IMAGES_URL}>go-next.png" alt="<{$next_product_title}>" /> <{$smarty.const._OLEDRION_NEXT_PRODUCT}>: <{$next_product_title}></a>
		<{/if}>

		<{if $better_together > 0 && $bestwith}>
			<br /><img src="<{$smarty.const.OLEDRION_IMAGES_URL}>icon-star.png" alt="<{$bestwith.product_title}>" /> <{$smarty.const._OLEDRION_BEST_WITH}> <a href="<{$bestwith.product_url_rewrited}>" title="<{$bestwith.product_href_title}>"><{$bestwith.product_title}></a>
		<{/if}>

		<{if count($product_all_categs) > 0}>
			<h2><img src="<{$smarty.const.OLEDRION_IMAGES_URL}>icon-product-person.png" alt="<{$smarty.const._OLEDRION_RECENT_CATEGS}>" /><{$smarty.const._OLEDRION_RECENT_CATEGS}></h2>
		...
		<{if count($product_current_categ) > 0}>
			<h2><img src="<{$smarty.const.OLEDRION_IMAGES_URL}>icon-product-person.png" alt="<{$smarty.const._OLEDRION_RECENT_CATEG}>" /><{$smarty.const._OLEDRION_RECENT_CATEG}></h2>
			<table class='center oledrion_lastproducts'>
			<tr>
	...
	<div id="oledrion_caddy" class="right">
		<br />
		<a href="<{$smarty.const.OLEDRION_URL}>caddy.php" title="<{$smarty.const._OLEDRION_CART}>"><img src="<{$smarty.const.OLEDRION_IMAGES_URL}>cart.png" alt="<{$smarty.const._OLEDRION_CART}>" /></a>&nbsp;
		<{if $mod_pref.rss}>
			<a href="<{$smarty.const.OLEDRION_URL}>rss.php" title="<{$smarty.const._OLEDRION_RSS_FEED}>"><img src="<{$smarty.const.OLEDRION_IMAGES_URL}>rss.gif" alt="<{$smarty.const._OLEDRION_RSS_FEED}>" /></a>&nbsp;
		<{/if}>
		<a href="<{$baseurl}>&op=print" rel="nofollow" target="_blank" title="<{$smarty.const._OLEDRION_PRINT_VERSION}>"><img src="<{xoModuleIcons16 printer.png}>" alt="<{$smarty.const._OLEDRION_PRINT_VERSION}>" /></a>&nbsp;
		<a href="<{$mail_link}>" rel="nofollow" target="_blank" title="<{$smarty.const._OLEDRION_TELLAFRIEND}>"><img src="<{xoModuleIcons16 mail_forward.png}>" alt="<{$smarty.const._OLEDRION_TELLAFRIEND}>" /></a>&nbsp;
		<{if $mod_pref.isAdmin}>
			<a href="<{$smarty.const.OLEDRION_URL}>admin/index.php?op=products&action=edit&id=<{$product.product_id}>" target="_blank" title="<{$smarty.const._EDIT}>"><img src="<{xoModuleIcons16 edit.png}>" alt="<{$smarty.const._EDIT}>" /></a>
			&nbsp;<a href="<{$smarty.const.OLEDRION_URL}>admin/index.php?op=products&action=confdelete&id=<{$product.product_id}>" title="<{$smarty.const._DELETE}>"><img src="<{xoModuleIcons16 delete.png}>" alt="<{$smarty.const._DELETE}>" /></a>
		<{/if}>
