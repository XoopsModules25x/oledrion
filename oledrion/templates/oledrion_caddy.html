<div class="oledrion">
	...
	<div class="center"><h2><img src="<{$smarty.const.OLEDRION_IMAGES_URL}>add-to-basket.png" alt="<{$smarty.const._MI_OLEDRION_SMNAME1}>" /><{$smarty.const._MI_OLEDRION_SMNAME1}></h2></div>

	<{if $emptyCart}>
		<{$smarty.const._OLEDRION_CART_IS_EMPTY}>
		<{if $isCartExists}>
			<div class="CartExists"><a href="<{$smarty.const.OLEDRION_URL}>caddy.php?op=reload"><{$smarty.const._OLEDRION_RELOAD_PERSISTENT}></a></div>
		<{/if}>
	<{else}>
		<form method="post" name="frmUpdate" id="frmUpdate" action="<{$smarty.const.OLEDRION_URL}>caddy.php" style="margin:0; padding:0; border: 0; display: inline;">
		...
		<tr>
			...
			<td><div class="oledrion_productprice right"><{$product.unitBasePriceFormated}></div></td>
			<td><div class="oledrion_productprice right"><{$product.discountedPriceFormated}></div></td>
			<td class="center"><input type="text" name="qty_<{$product.number}>" id="qty_<{$product.number}>" value="<{$product.product_qty}>" size="3" /></td>
			<td><div class="oledrion_productprice right"><{$product.discountedPriceWithQuantityFormated}></div></td>
			<td class='right'><{$product.vatRate}></td>
			<td><div class="oledrion_productprice right"><{$product.discountedShippingFormated}></div></td>
			<td><div class="oledrion_productprice right"><{$product.totalPriceFormated}></div></td>
			<td><a href="<{$smarty.const.OLEDRION_URL}>caddy.php?op=delete&product_id=<{$product.number}>" <{$confirm_delete_item}> title="<{$smarty.const._OLEDRION_REMOVE_ITEM}>"><img src="<{$smarty.const.OLEDRION_IMAGES_URL}>cartdelete.png" alt="<{$smarty.const._OLEDRION_REMOVE_ITEM}>" /></td>
		</tr>
				...
				<td colspan="10">
					...
					<{foreach item=attribute from=$product.attributes}>
						...
						<{foreach item=option from=$attribute.attribute_options}>
							<div class="attribute_options"><{$option.option_name}> : <{$option.option_price_ht_formated}> (<{$option.option_vat_formated}>), <{$option.option_ttc_formated}></div>
						<{/foreach}>
						</li>
					<{/foreach}>
					</ul>
		...
		<tr>
			<td colspan="8">
				...
				<form method="post" name="frmEmpty" id="frmEmpty" action="<{$smarty.const.OLEDRION_URL}>caddy.php" <{$confEmpty}> style="margin:0; padding:0; border: 0; display: inline;">
			...
			<td colspan="2" class="center">
				<{if $showOrderButton}>
					<form method="post" name="frmCheckout" id="frmCheckout" action="<{$smarty.const.OLEDRION_URL}>checkout.php" style="margin:0; padding:0; border: 0; display: inline;">
